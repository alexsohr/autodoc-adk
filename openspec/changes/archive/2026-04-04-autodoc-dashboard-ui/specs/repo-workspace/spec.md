...
### Requirement: Overview tab metric cards
The Overview tab SHALL display four metric cards at the top: Doc Pages (with delta since last generation), Avg Quality Score (with delta since last generation), Scopes (with list of scope paths), and Last Generated (with branch name and commit SHA).

#### Scenario: Metric cards displayed
- **WHEN** the user views the Overview tab for a repository with completed generations
- **THEN** four metric cards SHALL be rendered showing: Doc Pages count with delta indicator, Avg Quality Score with delta indicator, Scopes count with scope path list, and Last Generated timestamp with branch and short commit SHA

#### Scenario: Delta indicators
- **WHEN** the current generation produced 45 pages and the previous generation produced 40 pages
- **THEN** the Doc Pages metric card SHALL display "45" with a "+5" delta indicator in a positive (green) style

#### Scenario: No previous generation for delta
- **WHEN** only one generation has been completed for a repository
- **THEN** the delta indicators SHALL be absent or display "N/A"
